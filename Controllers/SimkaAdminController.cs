...
namespace APIConsume.Controllers
{
    ...
    public class SimkaAdminController : Controller
    {
        ...
        public IActionResult KelolaStudiLanjut()
        {
            dynamic balikan = new ExpandoObject();
                balikan.refJenjang = _context.RefJenjang.ToList();
                balikan.refStatusStudi = _context.RefStatusStudi.ToList();
                balikan.listUniversitas = (new TblStudiLanjutDAO()).GetListUniversitas().data;
                balikan.refSumberBiaya = _context.RefSumberBiaya.ToList();
        ...
        [Authorize(Roles = "Admin, KSDM")]
        public IActionResult KelolaSumberBiayaSL()
        {
                return View();
        ...
        [Authorize(Roles = "Admin, KSDM")]
        public IActionResult KelolaPersetujuanSB()
        {
        ...
        [Authorize(Roles = "Admin, KSDM")]
        public IActionResult KelolaPencairanYSR()
        {
        ...
        [Authorize(Roles = "Admin, KSDM")]
        public IActionResult IndexRekapPengajuanYSR()
        {
        ...
        [Authorize(Roles = "Admin, KSDM")]
        public async Task<IActionResult>  SinkronisasiperProdi()
        {
            dynamic balikan = new ExpandoObject();
            balikan.semesterSister = await getIdSemesterSister();
            balikan.mstUnit = (new MstUnitDAO()).GetNamaUnit();
                return View(balikan);
   
        }

        [Authorize(Roles = "Admin, KSDM")]
        public async Task<IActionResult> DaftarDosen()
        {
        ...
        public async Task<IActionResult> LoadDataKaryawanbyCriteria(int idJabatanAkademik, int idFungsional, int idUnit, int idSubUnit, string PendidikanTerakhir, string statusKepegawaian, string npp, string nama)
        {
            try
            {
                var customerData = _context.MstKaryawan
                ...
                DBOutput data = await (new MstKaryawanDAO()).GetDataKaryawanByCriteria(idJabatanAkademik, idFungsional, idUnit, idSubUnit, PendidikanTerakhir, statusKepegawaian, npp, nama);
        ...
        [Authorize(Roles ="Admin,KSDM")]
        public IActionResult AddEditSuratKeputusan(string nosk = null)
        {
        ...
        public IActionResult AddEditRiwayatPendidikan(int id = 0)
        {
            if (id == 0)
            {
                ...
                {
                    // mengambil  daftar jenis appraisal untuk dipopualte dalam select
                    karyawan = _context.MstKaryawan.Select(x => new MstKaryawan { Npp = x.Npp, Nama = x.Nama }).OrderBy(a => a.Nama).ToList(),
        ...
        [HttpPost]
        public async Task<IActionResult> AddEditRiwayatPendidikan(TrRiwayatPendidikanForm rp)
        {
                ...
                try
                {
                    ...
                    if (data.IdTrRp == 0)
                    {
                        ...
                        TempData["SuccessMessage"] = "Tambah Data Riwayat Pendidikan Berhasil";
                    }
                    else
                    {
                        _context.Update(data);
                        ...
                        TempData["SuccessMessage"] = "Ubah Data Riwayat Pendidikan Berhasil";
                    }

                    return RedirectToAction("KelolaRiwayatPendidikan","SimkaAdmin");
                }
                catch (Exception ex)
                {
                    TempData["ErrorMessage"] = ex.InnerException.Message;
        ...
        public async Task<IActionResult> ProfilDosen(string npp)
        {
                ...
                var lastupdatePenelitian = (new TblLogDAO()).GetLastUpdated("Sinkronisasi Penelitian Sister", npp);
                var lastupdatePengabdian = (new TblLogDAO()).GetLastUpdated("Sinkronisasi Pengabdian Sister", npp);
                var lastupdatePublikasi = (new TblLogDAO()).GetLastUpdated("Sinkronisasi Publikasi Sister", npp);
                var lastupdatePengajaran = (new TblLogDAO()).GetLastUpdated("Sinkronisasi Pengajaran Sister", npp);

                if (lastupdatePenelitian != new DateTime())
                    balikan.LastUpdatedPenelitian = (DateTime.Now - lastupdatePenelitian).Days;
                else
                    balikan.LastUpdatedPenelitian = null;

                if (lastupdatePengabdian != new DateTime())
                    balikan.LastUpdatedPengabdian = (DateTime.Now - lastupdatePengabdian).Days;
                else
                    balikan.LastUpdatedPengabdian = null;

                if (lastupdatePublikasi != new DateTime())
                    balikan.LastUpdatedPublikasi = (DateTime.Now - lastupdatePublikasi).Days;
                else
                    balikan.LastUpdatedPublikasi = null;

                if (lastupdatePengajaran != new DateTime())
                    balikan.LastUpdatedPengajaran = (DateTime.Now - lastupdatePengajaran).Days;
                else
                    balikan.LastUpdatedPengajaran = null;
        ...
        public async Task<IActionResult> PostPengembangan(TrPengembanganForm pengembangan)
        {
            ...
            if (!ModelState.IsValid)
            {
                return BadRequest(pengembangan);
            }
        ...
        public async Task<IActionResult> AddEditKarirStrukturalAsync(string npp, string nosk)
        {
            ...
            var karyawanada = await _context.TrKarirStruktural.AsNoTracking().AnyAsync(x => x.Npp == npp && x.NoSk == nosk);
                ...
                if (!karyawanada)
                {
                    ...
                    {
                        karyawan = _context.MstKaryawan.Select(x => new MstKaryawan { Npp = x.Npp, Nama = x.Nama }).OrderBy(c => c.Nama).ToList(),
                        unit = _context.MstUnit.Select(x => new MstUnit
                        {
                            IdUnit = x.IdUnit,
                            NamaUnit = x.NamaUnit
        ...
        public IActionResult LoadDataStudiLanjutbyCriteria(string npp, string nama, int idJenjang, int idSumberBiaya, string Universitas, string Fakultas, string ProgramStudi, string Kota, string Negara, DateTime tanggalMulai, DateTime tanggalLulus, int targetLulus, string DalamAtauLuarNegeri, int idRefSs)
        {
            try
            {
                DBOutput result = (new TblStudiLanjutDAO()).GetDataStudiLanjutByCriteria(npp, nama, idJenjang, idSumberBiaya, Universitas, Fakultas, ProgramStudi, Kota, Negara, tanggalMulai, tanggalLulus, targetLulus, DalamAtauLuarNegeri, idRefSs);
        ...
        public async Task<IActionResult> AddEditStudiLanjut(int id = 0)
        {
                ...
                else
                {
                    ...
                    var sumberBiaya = new TblSumberBiayaSLDAO().GetTblSumberBiaya(id);
                    var hiddenIdSumberBiayaSL = "";
                    var hiddenSumberBiaya = "";
                    var hiddenSumberBiayaKe = "";

                    //for (var i = 0; i < sumberBiaya.Count ; i++)
                    //{
                    //    if (i == 0)
                    //    {
                    //        hiddenIdSumberBiayaSL = sumberBiaya[i].ID_SUMBER_BIAYA_SL.ToString();
                    //        hiddenSumberBiaya = sumberBiaya[i].ID_SUMBER_BIAYA.ToString();
                    //        hiddenSumberBiayaKe = sumberBiaya[i].SUMBER_BIAYA_KE.ToString();
        ...
        public async Task<IActionResult> AddEditStudiLanjut(TblStudiLanjutForm studiLanjut)
        {
                ...
                try
                {
                    ...
                    if (studiLanjut.SkPenempatanKmbl != null && studiLanjut.SkPenempatanKmbl.Length > 0)
                    {
                        ...
                        studiLanjut.SkPenempatanKmblm = p1;
                    ...
                    return RedirectToAction("KelolaStudiLanjut","SimkaAdmin");
                }
                catch (Exception ex)
                {
                    TempData["ErrorMessage"] = ex.InnerException.Message;
        ...
        public JsonResult LoadDataSumberBiayaBySL(int idSL)
        {
            if (idSL == 0)
            {
                string[] a = new string[0];
            ...
            {
                var output = new TblSumberBiayaSLDAO().GetTblSumberBiaya(idSL);
                return Json(new { data = output });
            }

        }

        public JsonResult getRefSumberBiaya()
        {
            var data = _context.RefSumberBiaya.ToList();
            return Json(data);
        }

        //public JsonResult GetSumberBiayaByID_SL(int idSL)
        //{
        //    var data = new TblSumberBiayaSLDAO().GetTblSumberBiaya(idSL).data;
        //    return Json(data);
        //}

        public JsonResult GetSumberBiayaByIdSB(int idSB)
        {

            var output = new TblSumberBiayaSLDAO().GetTblSumberBiayaByIDSB(idSB);
            //output.buktiBase64 = Convert.ToBase64String(output.NO_BUKTI);

            //var data = _context.TblBiayaSlInternal.FirstOrDefault(a => a.IdSumberBiayaSl == idSB);
            return Json(output);
        }
        public async Task<int> PostSumberBiaya(int idSL, int idSB, int sumberBiaya) // Sementara tidak dipakai
        {
            int count = 0;
            if (idSB == 0)
            {

                TblSumberBiayaSl model = _context.TblSumberBiayaSl.OrderByDescending(a => a.SumberBiayaKe).FirstOrDefault(a => a.IdStudiLanjut == idSL);



                if (model == null) // Menentukan Sumber Biaya Ke berapa
                    count = 1;
                else
                {
                    count = model.SumberBiayaKe + 1;
                }

                var result = await new TblSumberBiayaSLDAO().AddTblSumberBiaya(idSL, sumberBiaya, count); // Add Sumber Biaya ke TBL Sumber Biaya
                idSB = _context.TblSumberBiayaSl.OrderByDescending(a => a.IdSumberBiayaSl).FirstOrDefault(a => a.IdStudiLanjut == idSL).IdSumberBiayaSl; // Mendapatkan ID Sumber biaya yang baru saja dimasukkan
            }
            else
            {
                var SBAda = _context.TblSumberBiayaSl.SingleOrDefault(a => a.IdSumberBiayaSl == idSB);
                SBAda.IdSumberBiaya = sumberBiaya;
                await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> PostSumberBiayaInternal(int idSL, int idSB, int idSBInternal, int sumberBiaya, int? tahun, string? semester, string? deskripsi, DateTime? tgl_pengajuan, decimal? jumlah_pengajuan, decimal? jumlah_dicairkan, DateTime? tgl_transfer, IFormFile Bukti)
        {
                ...
                var isInternal = _context.RefSumberBiaya.SingleOrDefault(a => a.IdSumberBiaya == sumberBiaya).IS_INTERNAL;
                idSB = await PostSumberBiaya(idSL, idSB, sumberBiaya); // Ambil id sumber biaya yang baru saja dimasukkan
                try
                {

                    TblBiayaSlInternal data = new TblBiayaSlInternal()
                    {
                        IdBiayaSlInt = idSBInternal,
                        IdSumberBiayaSl = idSB,
                        Tahun = tahun.ToString(),
                        Semester = semester,
                        Deskripsi = deskripsi,
                        TglPengajuan = tgl_pengajuan,
                        JumlahPengajuan = jumlah_pengajuan,
                        JumlahDicairkan = jumlah_dicairkan,
                    ...
                    {
                        byte[] p1 = null;
                        using (var fs1 = Bukti.OpenReadStream())
                        using (var ms1 = new MemoryStream())
                        {
                            fs1.CopyTo(ms1);
                            p1 = ms1.ToArray();
                    ...
                    {
                        _context.TblBiayaSlInternal.Update(data);
                        await _context.SaveChangesAsync();
                        return Json(new
                        {
                            success = true,
                            message = "Ubah Data Sumber Biaya Success",
                        });
                    }
                }
                catch (Exception ex)
        ...
        public async Task<IActionResult> DeleteSumberBiaya([FromRoute] int id)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            _context.TblSumberBiayaSl.Remove(todo);
            await _context.SaveChangesAsync();
        ...
        public async Task<IActionResult> DeleteStudiLanjut([FromRoute] int id)
        {
            ...
            var ListSB = new TblSumberBiayaSLDAO().GetTblSumberBiaya(todo.IdStudiLanjut);
            foreach (var item in ListSB)
            {
                await DeleteSumberBiaya(item.id_sumber_biaya_sl);
            }
            if (todo == null)
            {
                return NotFound();
        ...
        public IActionResult tesValidasi()
        {
            return View();
        }

        public JsonResult LoadDataListPersetujuanSB()
        {
            
            //var output = _context.TblBiayaSlInternal.Where(a => a.TglApprovalFakultas == null && a.NO_BUKTI == null).ToList();
            var output = new TblSumberBiayaSLDAO().GetTblSumberBiayaButuhApproval();
            return Json(new { data = output });
            

        }

        public async Task<JsonResult>  SBDisetujui(int idSBInt)
        {
            
            try
            {
                var data = _context.TblBiayaSlInternal.FirstOrDefault(a => a.IdBiayaSlInt == idSBInt);
                data.TglApprovalFakultas = DateTime.Now;
                await _context.SaveChangesAsync();
                return Json(new { success = true, message = "Sumber Biaya berhasil disetujui" });

            }catch(Exception ex)
            {
                return Json(new { success = false, message = "Sumber Biaya gagal disetujui" });
        ...
        public JsonResult LoadDataListPencairanYSR()
        {

            //var output = _context.TblBiayaSlInternal.Where(a => a.TglApprovalFakultas == null && a.NO_BUKTI == null).ToList();
            var output = new TblSumberBiayaSLDAO().GetTblSumberBiayaPencairanYSR();
            return Json(new { data = output });


        }

        public async Task<JsonResult> PostPencairanYSR(int idSBInt, decimal jml_pencairan, DateTime tgl_pencairan, IFormFile bukti)
        {

            try
            {
                var data = _context.TblBiayaSlInternal.FirstOrDefault(a=>a.IdBiayaSlInt == idSBInt);
                data.JumlahDicairkan = jml_pencairan;
                data.TglCair = tgl_pencairan;
                ...
                await _context.SaveChangesAsync();
                return Json(new { success = true, message = "Pencairan YSR berhasil " });

            }
            catch (Exception ex)
            {
                return Json(new { success = false, message = "Pencairan YSR gagal" });
        ...
        public JsonResult LoadDataRekapPengajuanYSR()
        {
            
                var output = new TblSumberBiayaSLDAO().GetRekapPengajuanYSR();
                return Json(new { data = output });
