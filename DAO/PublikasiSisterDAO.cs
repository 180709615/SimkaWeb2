...
namespace APIConsume.DAO
{
    public class PublikasiSisterDAO
    {
        ...
        public DBOutput GetPublikasiSisterTrPublikasi(string idDosen)
        {
            using (SqlConnection conn = new SqlConnection(Connection.DataSisterConnection))
            {
                ...
                try
                {
                    ...
                    //                WHERE(PENULIS.id_sdm = @id_dosen)            ";
                    string query = @"SELECT *
FROM     PUBLIKASI INNER JOIN
                  PENULIS ON PUBLIKASI.id_riwayat_publikasi_paten = PENULIS.id_riwayat_publikasi_paten
WHERE  (PENULIS.id_sdm = @id_dosen)";
                    var param = new { id_dosen = idDosen };
                    output.status = true;
                    output.data = conn.Query<TrPublikasi_DATA_SISTER>(query, param).ToList();
