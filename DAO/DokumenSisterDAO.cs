...
namespace APIConsume.DAO
{
	public class DokumenSisterDAO
	{
			...
			using (SqlConnection conn = new SqlConnection(Connection.DataSisterConnection))
			{
				try
				{
					string query = @"SELECT *
									...
									WHERE(PENELITIAN.id_penelitian_pengabdian = @id)            ";
		...
		public DBOutput GetDokumenPublikasiById_publikasi(string id1)
		{
			DBOutput output = new DBOutput();
			...
			using (SqlConnection conn = new SqlConnection(Connection.DataSisterConnection))
			{
				...
				{
					string query = @"SELECT *
									FROM     DOKUMEN INNER JOIN PUBLIKASI ON PUBLIKASI.id_riwayat_publikasi_paten = DOKUMEN.id_publikasi_atau_penelitian
									WHERE(PUBLIKASI.id_riwayat_publikasi_paten = @id)            ";
