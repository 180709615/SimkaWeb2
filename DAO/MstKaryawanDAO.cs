...
namespace APIConsume.DAO
{
	public class MstKaryawanDAO
	{
		...
		public async Task<DBOutput> GetDataKaryawanByCriteria(int idJabatanAkademik, int idFungsional, int idUnit, int idSubUnit, string PendidikanTerakhir, string statusKepegawaian, string npp, string nama)
		{
			DBOutput output = new DBOutput();
			var kondisi = "";
			...
			if (!String.IsNullOrEmpty(PendidikanTerakhir))
			{
				if (String.IsNullOrEmpty(kondisi))
					kondisi = " WHERE mstKaryawan.PENDIDIKAN_TERAKHIR = @PendidikanTerakhir ";
				else
					kondisi = kondisi + " AND mstKaryawan.PENDIDIKAN_TERAKHIR = @PendidikanTerakhir ";
