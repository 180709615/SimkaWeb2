    ...
    public class SimkaDosenController : Controller
    {
        ...
        public async Task<IActionResult> LoadDataPengajaranAsync(string id_semester, string npp)
        {
            ...
            if (HttpContext.Session.GetString("NPP") != null)
            {
                ...
                var data = (new PengajaranSisterDAO()).GetPengajaranTrPengajaran(npp, id_semester).data;
                return Json(new
                {
                    data
                });
        ...
        public async Task<IActionResult> SinkronDataPenelitianSISTERbyNPP(string npp)
        {
            ...
            List<TrPenelitian_DATA_SISTER> masterList =
                (from penelitian in _DATA_SISTERcontext.TrPenelitian_DATA_SISTER
                 join anggota in _DATA_SISTERcontext.TblAnggota_DATA_SISTER on penelitian.id_penelitian_pengabdian equals anggota.id_penelitian_pengabdian
                 where anggota.id_sdm == idDosen
                 select penelitian).ToList();// buat cari data yang NPP nya sama dengan Npp dosen saat ini
                                             // buat cari data yang NPP nya sama dengan Npp dosen saat ini
...

        {
            ...
            List<TrPengabdian_DATA_SISTER> masterList =
                ( from pengabdian in _DATA_SISTERcontext.TrPengabdian_DATA_SISTER
                join anggota in _DATA_SISTERcontext.TblAnggota_DATA_SISTER on pengabdian.id_penelitian_pengabdian equals anggota.id_penelitian_pengabdian
                where anggota.id_sdm == idDosen
                select pengabdian ).ToList();// buat cari data yang NPP nya sama dengan Npp dosen saat ini
            ...
            if (HttpContext.Session.GetString("NPP") != null)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    try
                    {
                        ...
                        if (!String.IsNullOrEmpty(idDosen) && idDosen.Length == 36)
                        {
                            var url = baseUrl + "/pengabdian?id_sdm=" + idDosen;
                            httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                            httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + TokenSister.token);
                            var response = await httpClient.GetAsync(url);
                            var apiResponse = await response.Content.ReadAsStringAsync();
                        ...
                        if (!String.IsNullOrEmpty(idDosen) && idDosen.Length == 36) // Cek apakah Id Dosen valid atau tidak
                        {
                            ...
                            if (response.IsSuccessStatusCode)// Jika daftar publikasi dari suatu dosen ditemukan dan tidak error
                            {
                                ...
                                if (ajar != null)
                                {
                                    ...
                                    foreach (Publikasi itemPublikasi in ajar) // Perulangan untuk mendapatkan detail publikasi dosen
                                    {
                                        ...
                                        if (response.IsSuccessStatusCode) // Jika detail publikasi berhasil didapatkan
                                        {
                                            ...
                                            TrPublikasi_DATA_SISTER entrydata = new TrPublikasi_DATA_SISTER() {
                                                id_riwayat_publikasi_paten = detail.id,
                                    ...
                                    toBeAdded = compareList.Where(a => !masterList.Any(b => a.id_riwayat_publikasi_paten == b.id_riwayat_publikasi_paten)).ToList();
                                    toBeDeleted = masterList.Where(a => !compareList.Any(b => a.id_riwayat_publikasi_paten == b.id_riwayat_publikasi_paten)).ToList();
                                    toBeUpdated = compareList.Where(a => masterList.Any(b => a.id_riwayat_publikasi_paten == b.id_riwayat_publikasi_paten)).ToList();
                                ...
                                DBOutput addTblLog = (new TblLogDAO()).AddTblLog(npp, 517, "Sinkronisasi Publikasi Sister");
        ...
        public async Task<IActionResult> SinkronDataPengajaranProdiSISTERbyNPP(string id_semester, string npp)
        {
            ...
            List<TrPengajaran_Data_SISTER> masterList = _DATA_SISTERcontext.TrPengajaran_DATA_SISTER.Where(a => a.NPP == npp && a.id_semester == id_semester).ToList();
                ...
                if (id_semester != null)
                {
                    using (var httpClient = new HttpClient())
                    {
                        try
                        {
                            ...
                            if (!String.IsNullOrEmpty(idDosen) && idDosen.Length == 36)
                            {
                                var url = baseUrl + "/pengajaran?id_sdm=" + idDosen + "&id_semester=" + id_semester;
                                httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + TokenSister.token);
                                var response = await httpClient.GetAsync(url);
                                var apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> ExportPublikasi(string npp)
        {
            byte[] result;
            if(npp == null)
                npp = HttpContext.Session.GetString("NPP");
        ...
        public async Task<IActionResult> SinkronRiwayatPendidikanSister(string npp)
        {
            ...
            List<TrRiwayatPendidikan> masterList = new List<TrRiwayatPendidikan>();

            List<TrRiwayatPendidikan> compareList = new List<TrRiwayatPendidikan>();
            ...
            if (HttpContext.Session.GetString("NPP") != null)
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    try
                    {
                        ...
                        if (!String.IsNullOrEmpty(idDosen) && idDosen.Length == 36) // Cek apakah Id Dosen valid atau tidak
                        {
                            var url = baseUrl + "/pendidikan_formal?id_sdm=" + idDosen;
                            httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                            httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + TokenSister.token);
                            var response = await httpClient.GetAsync(url);
                            var apiResponse = await response.Content.ReadAsStringAsync();
                            ...
                            if (response.IsSuccessStatusCode)// Jika daftar publikasi dari suatu dosen ditemukan dan tidak error
                            {
                                var ajar = JsonConvert.DeserializeObject<List<RiwayatPendidikanSister>>(apiResponse);

                                if (ajar != null)
                                {


                                    foreach (RiwayatPendidikanSister itemRiwayat in ajar) // Perulangan untuk mendapatkan detail publikasi dosen
                                    {
                                        index++;
                                        //if (index < 10)
                                        //    continue;
                                        url = baseUrl + "/pendidikan_formal/" + itemRiwayat.id; // API Call detail publikasi berdasarkan id_publikasi
                                        httpClient.DefaultRequestHeaders.Accept.Add(new MediaTypeWithQualityHeaderValue("application/json"));
                                        //httpClient.DefaultRequestHeaders.Add("Authorization", "Bearer " + TokenSister.token);
                                        response = await httpClient.GetAsync(url);
                                        apiResponse = await response.Content.ReadAsStringAsync();
                                        if (response.IsSuccessStatusCode) // Jika detail publikasi berhasil didapatkan
                                        {
                                            ...
                                            var detail = JsonConvert.DeserializeObject<RiwayatPendidikanSisterDetail>(apiResponse);


                                            TrRiwayatPendidikan entryItem = new TrRiwayatPendidikan()
                                            {

                                                IdRefJenjang = _context.RefJenjang.SingleOrDefault(a => a.Deskripsi == itemRiwayat.jenjang_pendidikan).IdRefJenjang,
                                                TahunLulus = itemRiwayat.tahun_lulus,
                                                NamaSekolah = itemRiwayat.nama_perguruan_tinggi,
                                                Gelar = itemRiwayat.gelar_akademik,
                                                ProgramStudi = itemRiwayat.bidang_studi,

                                                TahunMasuk = detail.tahun_masuk,
                                                NoIjazah = detail.nomor_ijazah,
                                                DateInserted = DateTime.Now,
                                                Npp = npp,
                                                IdRefSs = itemRiwayat.tahun_lulus != null ? _context.RefStatusStudi.SingleOrDefault(a=>a.Deskripsi == "Lulus").IdRefSs : _context.RefStatusStudi.SingleOrDefault(a => a.Deskripsi == "Aktif").IdRefSs
                                            ...
                                            if (_context.TrRiwayatPendidikan.Any(a => a.Npp == npp && a.IdRefJenjang == entryItem.IdRefJenjang && a.ProgramStudi == detail.bidang_studi || a.Fakultas == detail.bidang_studi))
                                                masterList.Add(_context.TrRiwayatPendidikan.SingleOrDefault(a => a.Npp == npp && a.IdRefJenjang == entryItem.IdRefJenjang && a.ProgramStudi == detail.bidang_studi || a.Fakultas == detail.bidang_studi));

                                            compareList.Add(entryItem);
                                        ...
                                        else // jika ID yang  dicari tidak bisa ditemukan detailnya 
                                        {
                                            ...
                                            // Masukkan data ke masterlist kalau ID saat ini sudah ada di dalam database(berarti akan menjadi update)
                                            if (_context.TrRiwayatPendidikan.Any(a => a.Npp == npp && a.IdRefJenjang == entryItem.IdRefJenjang && a.ProgramStudi == itemRiwayat.bidang_studi || a.Fakultas == itemRiwayat.bidang_studi))
                                                masterList.Add(_context.TrRiwayatPendidikan.SingleOrDefault(a => a.Npp == npp && a.IdRefJenjang == entryItem.IdRefJenjang && a.ProgramStudi == itemRiwayat.bidang_studi || a.Fakultas == itemRiwayat.bidang_studi));
                                    ...
                                    List<TrRiwayatPendidikan> toBeAdded = new List<TrRiwayatPendidikan>();
                                    List<TrRiwayatPendidikan> toBeDeleted = new List<TrRiwayatPendidikan>();
                                    List<TrRiwayatPendidikan> toBeUpdated = new List<TrRiwayatPendidikan>();

                                    toBeAdded = compareList.Where(a => !masterList.Any(b => a.Npp == npp && a.IdRefJenjang == b.IdRefJenjang && a.ProgramStudi == b.ProgramStudi || b.Fakultas == a.ProgramStudi)).ToList();
                                    toBeDeleted = masterList.Where(a => !compareList.Any(b => a.Npp == npp && a.IdRefJenjang == b.IdRefJenjang && a.ProgramStudi == b.ProgramStudi || a.Fakultas == b.ProgramStudi)).ToList();
                                    toBeUpdated = compareList.Where(a => masterList.Any(b => a.Npp == npp && a.IdRefJenjang == b.IdRefJenjang && a.ProgramStudi == b.ProgramStudi || b.Fakultas == a.ProgramStudi)).ToList();

                                    _context.TrRiwayatPendidikan.AddRange(toBeAdded);
                                    _context.TrRiwayatPendidikan.RemoveRange(toBeDeleted);
                                    int counter = 0;
                                    foreach (var item in toBeUpdated)
                                    {
                                        counter++;

                                        TrRiwayatPendidikan itemToUpdate = _context.TrRiwayatPendidikan.FirstOrDefault(a => a.Npp == npp && a.IdRefJenjang == item.IdRefJenjang && a.ProgramStudi == item.ProgramStudi || a.Fakultas == item.ProgramStudi);


                                        itemToUpdate.ProgramStudi = item.ProgramStudi;
                                        itemToUpdate.TahunLulus = item.TahunLulus;
                                        itemToUpdate.TahunMasuk = item.TahunMasuk;
                                        itemToUpdate.Gelar = item.Gelar;

                                        itemToUpdate.NamaSekolah = item.NamaSekolah;
                                        itemToUpdate.NoIjazah = item.NoIjazah != null ? item.NoIjazah : itemToUpdate.NoIjazah;
                                        itemToUpdate.IdRefSs = item.IdRefSs;
                                ...
                                DBOutput addTblLog = (new TblLogDAO()).AddTblLog(npp, 517, "Sinkronisasi Riwayat Pendidikan Sister");
                                if (addTblLog.status == false)
                                {
                                    return Json(new { success = false, nama = namaDosen, message = "Tambah Tbl Log gagal \n" + addTblLog.pesan });
                                }
                            ...
                            {
                                var ajar = JsonConvert.DeserializeObject<RiwayatPendidikanSister>(apiResponse);
        ...
        {
            var dokumen = (new DokumenSisterDAO()).GetDokumenPublikasiById_publikasi(id).data;
            
            List<file_object> list = new List<file_object>();
            ...
            using (var httpClient = new HttpClient())
            {
                try
                {
                    ...
                    foreach (TblDokumen_DATA_SISTER item in dokumen)
                    {
                        file_object result = new file_object();
                        
                        result.nama_file = item.nama_dokumen;
                        var url = baseUrl + "/dokumen/" + item.id_dokumen + "/download";

                        var response = await httpClient.GetAsync(url);
                        byte[] apiResponseMessage = await response.Content.ReadAsByteArrayAsync();

                        if (response.IsSuccessStatusCode)
                        {
                            result.file_byte = Convert.ToBase64String(apiResponseMessage);
        ...
        public async Task<IActionResult> UnduhArtikelPublikasi(string id)
        {
            using (var httpClient = new HttpClient())
            {
                try
        ...
        [HttpPost]
        public async Task<IActionResult> PostDokumenPendukung(IFormFile file_prosiding, IFormFile file_cek_similaritas, IFormFile file_PAK, IFormFile file_korespondensi, string id)
        {
            
            TrPublikasi_DATA_SISTER data = _DATA_SISTERcontext.TrPublikasi_DATA_SISTER.FirstOrDefault(a => a.id_riwayat_publikasi_paten == id);
            if(data!= null)
            {
                try
                {
        ...
        [HttpGet]
        public IActionResult GetDokumenPendukung(string jenis_tabel,string id)
        {
